...
void calendar_calendar_view_draw_part_begin_event_cb(lv_event_t * e)
{
	...
	if(dsc->part == LV_PART_ITEMS) {
		if(dsc->id < 7) {
			...
			dsc->label_dsc->font = &lv_font_Apple_16;
		} else if (lv_btnmatrix_has_btn_ctrl(obj, dsc->id, LV_BTNMATRIX_CTRL_DISABLED)) {
			dsc->label_dsc->color = lv_color_hex(0xd7d7d7);
			dsc->label_dsc->font = &lv_font_Apple_12;
			...
			dsc->rect_dsc->bg_color = lv_color_hex(0xee5253);
			...
			dsc->label_dsc->color = lv_color_hex(0xe2e2e2);
			dsc->label_dsc->font = &lv_font_Apple_12;
			dsc->rect_dsc->bg_opa = 255;
			dsc->rect_dsc->bg_color = lv_color_hex(0x341f97);
		} else {
		}
	}
}
  void calendar_calendar_view_event_handler(lv_event_t * e)
{
